...
import { FiCheck, FiClock, FiUsers, FiBarChart, FiShare2, FiHome, FiChevronRight } from 'react-icons/fi';
...
const CoursePage = () => {
    ...
    if (!courseData) return <div className="text-center py-20 text-white">Course not found</div>
        ...
        <div className="relative w-full bg-linear-to-br from-richblack-900 via-richblack-900 to-richblack-800 text-richblack-5 min-h-screen pt-24 overflow-hidden">
            ...
            <div className="relative z-10 overflow-hidden">
                ...
                <div className="relative z-10 py-8 w-full px-4 sm:px-6 lg:px-12 xl:px-20">
                    <nav className="flex items-center gap-2 text-sm text-richblack-300 mb-8 overflow-x-auto whitespace-nowrap pb-2 scrollbar-none">
                        <div className="flex items-center gap-1 hover:text-yellow-100 cursor-pointer transition-colors" onClick={() => navigate('/')}>
                            <FiHome /><span>Home</span>
                        </div>
                        <FiChevronRight className="shrink-0" />
                        <div className="flex items-center gap-1 hover:text-yellow-100 cursor-pointer transition-colors" onClick={() => navigate('/catalog/all-courses')}>
                            <span>Catalog</span>
                        </div>
                        <FiChevronRight className="shrink-0" />
                        <span className="text-yellow-100 font-medium truncate">{courseData?.courseName}</span>
                    </nav>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <div>
                                ...
                                <p className="text-base md:text-lg text-richblack-300 leading-relaxed max-w-3xl">{courseData?.courseDescription}</p>
                            </div>
                            ...
                            <div className="flex flex-wrap items-center gap-6 pt-6 border-t border-richblack-700/50">
                                ...
                                <div className="flex items-center gap-2 text-sm text-richblack-300">
                                    <div className="p-2 bg-richblack-700/50 rounded-lg"><FiUsers className="text-yellow-100 text-lg"/></div>
                            ...
                            <div className="inline-flex items-center gap-4 p-4 bg-richblack-800/40 rounded-2xl border border-richblack-700/50 backdrop-blur-sm shadow-xl">
                                ...
                                <div>
                                    ...
                                    <p className="text-lg font-bold text-white leading-none">{courseData?.instructor?.firstName} {courseData?.instructor?.lastName}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="lg:sticky lg:top-24 h-fit">
                            <div className="relative group">
                                <div className="absolute -inset-1 bg-linear-to-r from-yellow-100/30 to-blue-500/30 rounded-2xl blur opacity-25 group-hover:opacity-50 transition duration-1000"></div>
                                <div className="relative bg-richblack-900 border border-richblack-700/50 rounded-2xl p-6 shadow-2xl backdrop-blur-xl">
                                    {courseData?.thumbnail && (
                                        <div className="relative mb-6 group/img overflow-hidden rounded-xl border border-richblack-700 shadow-inner">
                                            <img src={courseData?.thumbnail} alt={courseData?.courseName} className="w-full h-48 object-cover group-hover/img:scale-105 transition-transform duration-700" />
                                    ...
                                    <div className="mb-6">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <p className="text-4xl font-black text-white">₹{courseData?.price}</p>
                                            <div className="px-2 py-0.5 bg-yellow-100/10 rounded border border-yellow-100/20 text-[10px] font-bold text-yellow-100 uppercase">Best Price</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4 pt-8 mt-6 border-t border-richblack-800">
                                        ...
                                        <div className="grid grid-cols-1 gap-4">
                                            ...
                                            <div className="flex items-center gap-3 group/feat cursor-pointer" onClick={() => { navigator.clipboard.writeText(window.location.href); alert("Link copied!"); }}>
                    ...
                    <div className="space-y-4">
                        {courseData?.courseContent?.map((section, index) => (
                            <div key={section._id} className="bg-richblack-800/60 rounded-2xl border border-richblack-700 overflow-hidden transition-all duration-300 hover:border-yellow-100/20">
                                <div className="flex items-center justify-between p-5 cursor-pointer bg-richblack-800 hover:bg-richblack-700/50 transition-all duration-300" onClick={() => toggleSection(section._id)}>
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-full bg-yellow-100/10 flex items-center justify-center text-yellow-100 font-bold shadow-inner">{index + 1}</div>
                                        <div>
                                            <h4 className="text-base md:text-lg font-bold text-richblack-5">{section.sectionName}</h4>
                                            <p className="text-richblack-400 text-xs flex items-center gap-1 mt-1"><MdOutlineOndemandVideo />{section.subSection?.length} Lectures</p>
                                        </div>
                                    </div>
                                    <div className="w-8 h-8 rounded-full bg-richblack-700 flex items-center justify-center text-yellow-100">
                                ...
                                <div className={`transition-all duration-500 ease-in-out ${activeSections.includes(section._id) ? 'max-h-[1000px] border-t border-richblack-700 opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
                                    <div className="p-4 space-y-2 bg-richblack-900/30">
                                        {section.subSection?.map((sub, idx) => (
                                            <div key={sub._id} className="flex items-center gap-4 p-4 rounded-xl hover:bg-richblack-700/30 transition-all duration-200 group/sub">
                                                <div className="w-8 h-8 rounded-full border border-richblack-600 flex items-center justify-center text-richblack-400 text-xs font-bold group-hover/sub:text-yellow-100">{idx + 1}</div>
                                                <div className="flex-1">
                                                    <p className="text-sm font-semibold text-richblack-50 group-hover/sub:text-yellow-50">{sub.title}</p>
                                                    <div className="flex items-center gap-3 mt-1.5">
                                                        <span className="flex items-center gap-1.5 text-richblack-400 text-[10px] uppercase font-bold tracking-widest"><FiClock className="text-yellow-100 text-xs"/>{formatDuration(sub.timeDuration)}</span>
                                                    </div>
                                                </div>
                                                <div className="p-2 bg-richblack-700 rounded-lg group-hover/sub:bg-yellow-100/10 transition-colors">
                                                    <MdOutlineOndemandVideo className="text-richblack-400 group-hover/sub:text-yellow-100" />
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                    ...
                    <div className="w-full px-4 sm:px-6 lg:px-12 xl:px-20 text-center">
                        ...
                        <div className="flex flex-wrap justify-center gap-4">
                            {courseData?.tag?.map((tag, index) => (
                                <div key={index} className="group px-6 py-3 bg-richblack-900/60 border border-richblack-700 rounded-full hover:border-yellow-100/50 hover:bg-yellow-100/5 transition-all duration-300 transform hover:-translate-y-1 shadow-lg">
                                    <div className="flex items-center gap-2">
                                        <FiBarChart className="text-yellow-100 text-lg"/><span className="font-bold text-sm text-richblack-100 group-hover:text-yellow-100">{tag}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            {courseData?.ratingAndReviews?.length > 0 && (
                <div className="py-20 w-full px-4 sm:px-6 lg:px-12 xl:px-20 relative z-10">
                    ...
                    <Swiper slidesPerView={1} spaceBetween={25} loop={true} autoplay={{ delay: 3000 }} navigation={true} modules={[FreeMode, Pagination, Autoplay, Navigation]} breakpoints={{ 640: { slidesPerView: 1.5 }, 1024: { slidesPerView: 2.5 }, 1280: { slidesPerView: 3 } }} className="pb-12">
            ...
            <div className="py-20 bg-linear-to-b from-transparent to-richblack-900 w-full px-4 sm:px-6 lg:px-12 xl:px-20 relative z-10">
                <div className="relative group overflow-hidden rounded-4xl border border-richblack-700/50 bg-richblack-800 p-1">
                    ...
                    <div className="relative flex flex-col items-center text-center py-16 px-8 rounded-[1.95rem] bg-richblack-900">
                        ...
                        <h3 className="text-3xl md:text-5xl font-black text-white mb-6 leading-tight">Transform Your Career with <br/> <span className="text-yellow-100 italic">Industry-Standard</span> Excellence</h3>
                        <p className="text-richblack-400 text-lg mb-10 max-w-2xl leading-relaxed">Join our thriving community and start learning immediately.</p>
                        <div className="w-full max-w-sm"><BuyCourseButton course={courseData} /></div>
                    </div>
                </div>
            </div>
            ...
            <div className="py-10 border-t border-richblack-700/50 w-full px-4 sm:px-6 lg:px-12 xl:px-20 relative z-10">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                    ...
                    <div className="group transition"><p className="text-yellow-100 text-xl font-bold">{avgReview}</p><p className="text-richblack-400 text-xs">Rating</p></div>
                    <div className="group transition"><p className="text-yellow-100 text-xl font-bold">∞</p><p className="text-richblack-400 text-xs">Lifetime Access</p></div>
                </div>
            </div>
        </div>
    );
